            ...
            with open(full_path,"rb") as toml:
                ...
                if alert['rule']['threat'][0]['framework'] == "MITRE ATT&CK":
                    for threat in alert['rule']['threat']:
                        ...
                        filtered_object = {'tactic' : str(tactic), 'technique_id' : technique_id, 'technique_name' : technique_name, 'subtechnique_id': subtechnique_id, 'subtechnique_name' : subtechnique_name}
                        filtered_object_array.append(filtered_object)
                        alert_data[file] = filtered_object_array

mitre_tactic_list = ['none','reconnaissance','resource development','initial Aaccess','execution','persistence','privilege escalation','defense evasion','credential access','discovery','lateral movement','collection','command and control','exfiltration','impact']
    ...
    for line in alert_data[file]:
        ...
        try:
            ...
            if alert_name != mitre_name:
                print('Technique id and name mismatch in ' + file + ' Expected ' + '\'' + mitre_name + '\'' + ' given: ' + '\'' + alert_name + '\'')
        except KeyError:
            pass
